...
using static ConnectionRastr.Extension.CalcSetting;

namespace ConnectionRastr.Extension.Weighting
{
    public static class Weighting
    {
        ...
        private const string WEIGHTING = "ut_node";
        private const string WEIGHTING_TYPE = "tip";
        private const string WEIGHTING_NUMBER = "ny";
        private const string WEIGHTING_ACTIVE_POWER_LOAD = "pn";
        private const string WEIGHTING_REACTIVE_POWER_LOAD = "qn";
        private const string WEIGHTING_TANGENT = "tg";
        private const string WEIGHTING_ACTIVE_POWER_GENERATION = "pg";
        private const string WEIGHTING_REACTIVE_POWER_GENERATION = "qg";
        ...
        //public static void Weight2(this RastrWrapper provider)
        //{
        ...
        //                if ((kd == 0 && rastr.ut_Param[ParamUt.UT_ADD_P] == 0) 
        //                    || rastr.ut_Param[ParamUt.UT_TIP] == 1
        //                )
        //                {
        //                    rastr.AddControl(-1, "");
        ...
        public static void IncreaseStepWeight(
            this RastrWrapper wrapper,
            double factor
        )
        {
            var weighting = wrapper.Tables[WEIGHTING];
            var pn = weighting.Columns[WEIGHTING_ACTIVE_POWER_LOAD];
            var qn = weighting.Columns[WEIGHTING_REACTIVE_POWER_LOAD];
            var pg = weighting.Columns[WEIGHTING_ACTIVE_POWER_GENERATION];
            var qg = weighting.Columns[WEIGHTING_REACTIVE_POWER_GENERATION];

            for (int i = 0; i < weighting.Count; i++)
            {
                pn[i] = (double)pn[i] * factor; 
                qn[i] = (double)qn[i] * factor; 
                pg[i] = (double)pg[i] * factor; 
                qg[i] = (double)qg[i] * factor; 
            }
        }
    }
}
